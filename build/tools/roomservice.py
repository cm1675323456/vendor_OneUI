...
# Copyright (C) 2012-2014 ONEUI Project
#
...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from ONEUI Github (https://github.com/OneUI-Devices)." % device)
...
while not depsonly:
    try:
        request = urllib.request.Request("https://api.github.com/users/OneUI-Devices/repos?page=%d" % page)
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/oneui_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    f = open('.repo/local_manifests/oneui_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    dependencies_path = repo_path + '/oneui.dependencies'
    syncable_repos = []
...
print("Repository for %s not found in the OneUI-Devices Github repository list. If this is in error, you may need to manually add it to your local_manifests/oneui_manifest.xml." % device)